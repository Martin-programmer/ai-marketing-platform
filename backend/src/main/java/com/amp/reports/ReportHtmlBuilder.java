...
public class ReportHtmlBuilder {
    ...
    /**
     * Build the full HTML report (backwards-compatible, no branding).
     */
    public static String buildHtml(
        ...
        return buildHtml(clientName, agencyName, periodStart, periodEnd,
                current, previous, dailyData, topCampaigns, narrative, null, null);
    }

    /**
     * Build the full HTML report with agency branding.
     *
     ...
     * @param dailyData     daily breakdown data
     * @param topCampaigns  top campaigns data
     * @param narrative     optional AI narrative sections (nullable)
     * @param branding      optional agency branding (nullable — falls back to defaults)
     * @param poweredByText optional "Powered by" text (nullable)
     */
    public static String buildHtml(
            String clientName,
            String agencyName,
        ...
        String primaryColor = branding != null && branding.primaryColor() != null
                ? branding.primaryColor() : "#1976D2";
        String logoUrl = branding != null ? branding.logoUrl() : null;
        String reportFooter = branding != null && branding.reportFooterText() != null
                ? branding.reportFooterText() : null;
        String disclaimer = branding != null && branding.reportDisclaimer() != null
                ? branding.reportDisclaimer()
                : "Data sourced from Meta Ads. Last 1-2 days may contain preliminary data subject to change.";

        html.append("<!DOCTYPE html><html><head>");
        html.append("<meta charset='UTF-8'/>");
        html.append("<style>");
        html.append(getStyles(primaryColor));
        html.append("</style>");
        html.append("</head><body>");

        // Header with optional agency logo
        html.append("<div class='header'>");
        if (logoUrl != null && !logoUrl.isBlank()) {
            html.append("<img src='").append(logoUrl)
                .append("' alt='").append(escapeHtml(agencyName))
                .append("' style='max-height:60px; margin-bottom:12px;' /><br/>");
        }
        html.append("<h1>").append(escapeHtml(agencyName)).append("</h1>");
        html.append("<div class='subtitle'>Performance Report — ").append(escapeHtml(clientName)).append("</div>");
        ...
        if (reportFooter != null && !reportFooter.isBlank()) {
            html.append("<p>").append(escapeHtml(reportFooter)).append("</p>");
        } else {
            html.append("<p>Generated by ").append(escapeHtml(agencyName)).append("</p>");
        }
        html.append("<p class='disclaimer'>").append(escapeHtml(disclaimer)).append("</p>");
        if (poweredByText != null && !poweredByText.isBlank()) {
            html.append("<p class='powered-by'>").append(escapeHtml(poweredByText)).append("</p>");
        }
        html.append("</div>");
    ...
    private static String getStyles(String primaryColor) {
        return """
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body { font-family: 'Helvetica Neue', Arial, sans-serif; color: #333; padding: 40px; max-width: 900px; margin: 0 auto; }
            .header { text-align: center; margin-bottom: 40px; padding-bottom: 20px; border-bottom: 2px solid {{PRIMARY}}; }
            .header h1 { font-size: 28px; color: {{PRIMARY}}; margin-bottom: 8px; }
            .header .subtitle { font-size: 20px; color: #555; margin-bottom: 4px; }
            .header .period { font-size: 14px; color: #888; }
            .section { margin-bottom: 32px; }
            .section h2 { font-size: 18px; color: {{PRIMARY}}; margin-bottom: 16px; padding-bottom: 8px; border-bottom: 1px solid #E0E0E0; }
            ...
            .data-table th { background: {{PRIMARY}}; color: white; padding: 10px 12px; text-align: left; }
            ...
            .powered-by { font-size: 9px; color: #CCC; margin-top: 8px; }
